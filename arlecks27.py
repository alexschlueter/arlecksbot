...
class Arlecks30Commander(Commander):
    """
    ...
    def initialize(self):
       ...
       #     self.graph.add_edge(self.node_EnemyBaseToScoreIndex, vertex, weight = 1.0)
        ...
        anker = self.midOurSpawn + spawnConnection / 3
    ...
    def tick(self):
        ...
        if self.game.team.flag.carrier and self.game.team.flag.carrier.position:
            self.targets[self.game.team.flag.carrier] = (self.game.team.flag.position, 0.0)
        for enemy in self.game.enemyTeam.members:
            if enemy.position is not None and enemy.health > 0:
                if enemy.facingDirection is not None and enemy.seenlast is not None and enemy.state is not None:
                    ...
                    if pos is not None:
                        self.targets[enemy] = (pos, enemy.seenlast)
                    else:
                        self.targets[enemy] = (enemy.position, enemy.seenlast)
                else:
                    #sys.stdout.write('dumb\n')
                    self.targets[enemy] = (enemy.position, 0.0)
    ...
    def tick(self):
        ...
        for c in self.children:
            ...
            if c.state is Node.STATE_RUNNING:
                ...
                return
            elif c.state is Node.STATE_FAILED:
                self.state = Node.STATE_FAILED
...
class BotHasFlag(Condition):
    def test(self):
        return self.bot() == self.commander().game.enemyTeam.flag.carrier
...
class AntiHold(Node):
    ...
    def tick(self):
        if not self.target or self.bot().state != BotInfo.STATE_ATTACKING:
            enemy = random.choice([b for b in self.bot().visibleEnemies])
            self.target = self.commander().level.findRandomFreePositionInBox((self.bot().position-5.0, self.bot().position+5.0))
            self.commander().smartIssue(commands.Attack, self.bot(), self.target, 'anti hold', enemy.position)
            self.state = Node.STATE_RUNNING
    ...
    def tick(self):
        if not self.target:
            if self.commander().targets:
                ...
                kill = sorted(self.commander().targets.iteritems(), key = lambda f : f[1][0].distance(bot.position) + f[1][1])[0]
    
                if kill[1][0].distance(bot.position) + kill[1][1] < 1.5 * self.commander().level.firingDistance:
                    if kill[1][0].distance(bot.position) > self.commander().level.runningSpeed * 5.0:
                        ...
                        self.commander().smartIssue(commands.Charge, bot, kill[1][0], 'charge target')
                    else:
                        self.commander().smartIssue(commands.Attack, bot, kill[1][0], 'attack target')
                    self.target = kill
                    self.state = Node.STATE_RUNNING
                    return
            self.state = Node.STATE_FAILED
        elif self.target[0].health <= 0 or self.target not in self.commander().targets:
...
class Camp(Node):
    ...
    def tick(self):
        if not self.target or self.bot().state != BotInfo.STATE_CHARGING:
            ...
            self.commander().issue(commands.Defend, self.bot(), self.commander().midEnemySpawn, 'defend camp')
            self.state = Node.STATE_SUCCEEDED
            self.target = None
        else:
            self.state = Node.STATE_RUNNING 
